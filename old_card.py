...
class ScoreCardConfig:
    ...
    # Failure List EDWSQL41.us.lmco.com\\SSAPPPRD 
    
    
    sql_server:             str = "edwsql41.us.lmco.com\ssedwprd"              # SQL server name EDWSQL40.us.lmco.com  Nestler.us.lmco.com UMHOLTZ.us.lmco.com
    sql_database:           str = "SubcontractScorecard"                        # Target database
    sql_uid:                str = "ACE_TEAM"                                    # SQL user
    sql_pwd:                str = "zWQE25_vW8FcP"                               # SQL password zWQE25_vW8FcP
...
class ScoreCardTextPrep:
    ...
    def build_sid_history_two_horizon(state: ScoreCardState) -> None:
        ...
        Constructs per-SID sliding windows with TWO future targets:
            ...
            Horizon 1 (H1): next card
            Horizon 2 (H2): next-next card

        IMPORTANT SEMANTIC RULE:
        - All features and metadata must reflect knowledge available
        at the time of the LAST INPUT NOTE (t-1).
        - No metadata is pulled from future notes.
        ...
        for sid, notes in sid_to_notes.items():
            ...
            # Need at least 4 past notes to form an input window
            if total_notes < 4:
                continue

            # Sliding anchor: index of LAST input note
            for i in range(3, total_notes):
                ...
                # Explicit input-only semantics
                entry["color_set"] = color_seq
                entry["all_green"] = int(color_seq == "GGGG")
                entry["input_color_set"] = color_seq
                entry["input_all_green"] = entry["all_green"]
                ...
                # --- Metadata (STRICTLY from last input note) ---
                entry.update({
