...
class ScoreCardTextPrep:
    ...
    def build_sid_history_two_horizon(state: ScoreCardState) -> None:
        """
        Constructs per-SID sliding windows with TWO future targets:
            - Horizon 1: next card
            - Horizon 2: next-next card

        Inputs:
            - Exactly 4 past notes when possible

        Targets:
            - target_h1: label at t
            - target_h2: label at t+1

        Trainability rules:
            - trainable_h1: total_notes >= 5
            - trainable_h2: total_notes >= 6

        This function ONLY constructs ground truth.
        No modeling, no prediction, no inference logic.
        """

        df = state.enriched_df.copy()

        # --- Group notes by SID in chronological order ---
        ...
        for sid, notes in sid_to_notes.items():
            ...
            # We need at least 4 past notes to form an input window
            if total_notes < 4:
                continue

            # Sliding anchor: index of the LAST input note
            # Inputs = notes[i-3 : i+1]
            for i in range(3, total_notes):
                input_notes = notes[i-3:i+1]

                # --- Future notes ---
                h1_note = notes[i + 1] if i + 1 < total_notes else None
                h2_note = notes[i + 2] if i + 2 < total_notes else None

                trainable_h1 = h1_note is not None and total_notes >= 5
                trainable_h2 = h2_note is not None and total_notes >= 6

                entry: dict[str, Any] = {
                    "sid": sid,
                    "total_notes": total_notes,
                    "trainable_h1": trainable_h1,
                    "trainable_h2": trainable_h2,
                }
                ...
                entry["all_green"] = int(color_seq == "GGGG")

                last_input_note = input_notes[-1]
                entry["last_sid"] = last_input_note["sid_key"]

                # --- Horizon 1 target ---
                entry["target_h1"] = h1_note["target"] if h1_note is not None else -1
                entry["Color_Code_h1"] = h1_note["Overall"] if trainable_h1 else ""

                # --- Horizon 2 target ---
                entry["target_h2"] = h2_note["target"] if h2_note is not None else -1
                entry["Color_Code_h2"] = h2_note["Overall"] if trainable_h2 else ""

                # --- Metadata copied from H1 when available, otherwise fallback ---
                meta_source = h1_note if h1_note is not None else last_input_note

                entry.update({
                    "LM_Vendor_ID":                meta_source["LM_Vendor_ID"],
                    "Performance_Management_Type": meta_source["Performance_Management_Type"],
                    "PO_Complexity_Level":         meta_source["PO_Complexity_Level"],
                    "PO_Contract_Dollars_Mil":     meta_source["PO_Contract_Dollars_Mil"],
                    "Supplier_Name":               meta_source["Supplier_Name"],
                    "LOB_Name":                    meta_source["LOB_Name"],
                })
