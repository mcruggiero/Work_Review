...
class ScoreCardConfig:
    ...
    sql_pwd:                str = os.getenv("SPARK_SQL_PWD")                    # SQL password
...
class ScoreCardModeling:
    ...
    def prepare_dataset(
        ...
        Args:
            ...
            config_row (dict[str, Any]): A row from the model grid containing:
                ...
                - 'sampling_strategy'
                - 'vectorizer_strategy'
            horizon (Horizon): The prediction horizon (H1 or H2). Determines which
                trainability and target columns to use.
        ...
        # Filter to rows with sufficient history and marked as trainable for this horizon
        sid_df = sid_df[
    ...
    def find_best_model(self, horizon: Horizon = Horizon.H1) -> None:
        ...
        Process:
            - Calls prepare_dataset(...) for each model config with the specified horizon.
            ...
            - Selects the best model for this horizon and saves it to:
                - self.state.models_by_horizon[horizon]
                - self.state.best_model_by_horizon[horizon]
                - self.state.best_model_key_by_horizon[horizon]

        For H1 (backward compatibility), also sets:
            - self.state.best_model
            - self.state.best_model_key
            - self.state.models

        Args:
            horizon (Horizon): The prediction horizon to train models for.
        """
        ...
        # For H1 backward compatibility, also store in legacy location
        if horizon == Horizon.H1:
        ...
        assert self.state.best_model_key_by_horizon.get(h_int), f"Error! No model found for horizon H{h_int}"
    ...
    def train_one_model(
        ...
        """
        Trains a logistic regression model on a single dataset/config for a specific horizon.
        Returns model ID and result dictionary.
        Raises immediately on error — no silent failures.

        Args:
            dataset_key: String identifier for the dataset configuration
            dataset: Dict containing X, y, and metadata
    ...
    def predict_with_best_model(
        ...
        Args:
            ...
            horizon (Horizon): The prediction horizon (H1 or H2). Determines which
                best model to use and how to name output columns.

        Returns:
            pd.DataFrame: A copy of sid_df with appended columns.
            For H1 (backward compatible):
                - 'predicted_label', 'prob_green', 'prob_yellow', 'prob_red'
            For H2:
                - 'predicted_label_h2', 'prob_green_h2', 'prob_yellow_h2', 'prob_red_h2'

        Raises:
            ValueError: If no model has been trained for the specified horizon.
        """
        h_int = int(horizon)
        ...
        # Fallback to legacy location for H1 if horizon-specific not found
        ...
        # Column naming based on horizon (H1 uses original names for backward compat)
        if horizon == Horizon.H1:
    ...
    def load_best_model_by_key(
        ...
        Args:
            model_key (str): A model identifier string in one of these formats:
                New format: "H{n} | feature_set | sampling_strategy | vectorizer_strategy | {class_weights_dict}"
                Legacy format: "feature_set | sampling_strategy | vectorizer_strategy | {class_weights_dict}"
            horizon (Horizon): The horizon for this model (used if not in key).
        ...
        if parts[0].startswith("H") and parts[0][1:].isdigit():
            ...
            assert len(parts) == 5, f"Model key has wrong format: {model_key}"
        else:
    ...
    def merge_data(self) -> None:
        ...
        Column naming convention:
            - H1 uses original names: predicted_label, prob_green, prob_yellow, prob_red, predicted_color
            - H2 uses suffix: predicted_label_h2, prob_green_h2, prob_yellow_h2, prob_red_h2, predicted_color_h2
        """
        enriched_df = self.state.enriched_df.copy()

        # Start with H1 predictions (using legacy location for backward compatibility)
        h1_df = self.state.predictions_df_by_horizon.get(1) or self.state.predictions_df

        if h1_df is None:
            self.conn.report("JOIN", "No H1 predictions available for merge")
            return

        # Base columns to merge (shared across windows)
        base_cols = [
            "last_sid",
            "sid_key_0",
            "sid_key_1",
        ...
        # H1 prediction columns (original names)
        h1_pred_cols = [
            "predicted_label",
            "prob_green",
            "prob_yellow",
        ...
        if h2_df is not None:
            # H2 prediction columns (with _h2 suffix)
            h2_pred_cols = [
                "predicted_label_h2",
                "prob_green_h2",
                "prob_yellow_h2",
            ...
            # Merge H2 on last_sid (already in complete_df from H1 merge)
            ...
            # Drop duplicate last_sid column if created
...
class ScoreCardPipeline:
    ...
    def _stage_3_modeling_and_prediction(self) -> None:
        ...
        for horizon in SUPPORTED_HORIZONS:
            ...
            # Use hardcoded model key for each horizon (same config, different targets)
            # The model key format for H1 supports legacy format for backward compatibility
            if horizon == Horizon.H1:
                model_key = "complete_main_words_only | no_downsample_weighted | count | {0: 0.5, 1: 1.35, 2: 1.15}"
            else:
                model_key = f"H{h_int} | complete_main_words_only | no_downsample_weighted | count | {{0: 0.5, 1: 1.35, 2: 1.15}}"
        ...
        # Upload H1 predictions (for backward compatibility with existing ES index)
